...
vec3 skyGradient(vec3 worldDir) {
    ...
    vec3 col = mix(horizon, mid, pow(h, 3.2));
    col = mix(col, zenith, pow(h, 1.35) * 0.92);
    // 地表以下给一点暗色, 避免看到天空的底部
    return mix(col, horizon * 0.42, smoothstep(0.0, -0.14, worldDir.y));
}
...
vec3 sunScatter(vec3 worldDir, vec3 lightDir, vec3 lightColor) {
    ...
    return lightColor * (forward + around) * smoothstep(-0.10, 0.35, lightDir.y) * 0.55;
}

// 下界的天空被岩顶与雾遮蔽, 只留下层叠的暗红
vec3 netherSky(vec3 worldDir) {
    float h = clamp(worldDir.y * 0.5 + 0.5, 0.0, 1.0);
    vec3 col = mix(vec3(0.20, 0.055, 0.026), vec3(0.055, 0.026, 0.040), pow(h, 1.4));
    // 缓慢滚动的烟尘
    float smoke = fbm3(worldDir * 3.4 + vec3(frameTimeCounter * 0.006, 0.0, 0.0));
    return col * (0.82 + smoke * 0.36) * 0.85;
}
...
vec3 endSky(vec3 worldDir) {
    ...
    // 稀疏的紫光尘埃
    vec3 cell = floor(worldDir * 46.0);
    vec3 f = fract(worldDir * 46.0) - 0.5;
    if (hash13(cell) > 0.9955) {
...
vec3 renderSky(vec3 worldDir, bool includeStars) {
    ...
    col += sunScatter(worldDir, normalize(sunPosition), sunLightColor());
...
#else
    // 关闭程序化天空时只保留两层色带, 不加散射与星空
    vec3 col = skyGradient(worldDir);
#endif

    return applyWeatherToSky(col, smoothstep(0.25, -0.05, worldDir.y));
}
...
vec3 renderCelestial(vec3 worldDir) {
    ...
    if (moonVisible > 0.001) {
        ...
        col += celestialDisk(worldDir, moonDir, moonLightColor() * 1.5, 0.0185, 2.4) * moonVisible * max(phase, 0.12);
    }
    return col;
}
